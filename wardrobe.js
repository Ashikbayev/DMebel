...
let matChoice={
  ...
  ldspPrice:0,
  mdfPlenkaName:'', // МДФ Плёнка — своя марка/цена, независимо от Краски
  mdfPlenkaPrice:0,
  mdfKraskaName:'', // МДФ Краска — своя марка/цена, независимо от Плёнки
  mdfKraskaPrice:0,
  hingeBrand:'En-7',
  slideBrand:'En-7',
  slideType:'Телескоп',
};
// Материал фасада секции теперь трёхвариантный: 'ldsp' | 'mdfPlenka' | 'mdfKraska'.
// 'mdf' — старое значение из проектов, сохранённых до этой доработки; для
// обратной совместимости трактуется как 'mdfPlenka' (raньше mdfType по
// умолчанию был 'plenka').
function normFacadeMat(m){
  if(m==='mdf') return 'mdfPlenka';
  if(m==='mdfPlenka'||m==='mdfKraska'||m==='ldsp') return m;
  return 'ldsp';
}
function isMdfMaterial(m){ const n=normFacadeMat(m); return n==='mdfPlenka'||n==='mdfKraska'; }
...
function loadCatalog(){
  try{
    ...
    if(s){const d=JSON.parse(s);
      ...
      // Миграция старого формата (один общий mdfType/mdfName/mdfPrice)
      if(d.matChoice && d.matChoice.mdfName && !matChoice.mdfPlenkaName && !matChoice.mdfKraskaName){
        if(d.matChoice.mdfType==='kraska'){
          matChoice.mdfKraskaName=d.matChoice.mdfName; matChoice.mdfKraskaPrice=d.matChoice.mdfPrice||0;
        } else {
          matChoice.mdfPlenkaName=d.matChoice.mdfName; matChoice.mdfPlenkaPrice=d.matChoice.mdfPrice||0;
        }
      }
    }
  }catch(e){}
...
async function loadFromSheets(){
  ...
  try{
    ...
    if(!matChoice.mdfPlenkaName && catalog.facadePlenka && catalog.facadePlenka.length){
      matChoice.mdfPlenkaName=catalog.facadePlenka[0].name;
      matChoice.mdfPlenkaPrice=catalog.facadePlenka[0].price;
    }
    if(!matChoice.mdfKraskaName && catalog.facadeKraska && catalog.facadeKraska.length){
      matChoice.mdfKraskaName=catalog.facadeKraska[0].name;
      matChoice.mdfKraskaPrice=catalog.facadeKraska[0].price;
    }
...
function renderMatCards(){
  ...
  if(mdfCard){
    if(!catalog.loaded){
      ...
      const plenkaOpts=(catalog.facadePlenka||[]).map(function(d){
        return '<option value="'+d.name+'" '+(matChoice.mdfPlenkaName===d.name?'selected':'')+'>'+d.name+' — '+d.price.toLocaleString('ru-RU')+' ₸/м²</option>';
      }).join('');
      const kraskaOpts=(catalog.facadeKraska||[]).map(function(d){
        return '<option value="'+d.name+'" '+(matChoice.mdfKraskaName===d.name?'selected':'')+'>'+d.name+' — '+d.price.toLocaleString('ru-RU')+' ₸/м²</option>';
      }).join('');
      mdfCard.innerHTML='<div class="price-card-title"><span style="color:#fff0d4;border:1px solid #ccc">■</span> МДФ фасад</div>' +
        '<div class="fl">Плёнка (марка)</div><select class="cat-select" onchange="setMdfPlenkaName(this.value)">'+plenkaOpts+'</select>' +
        '<div class="cat-price-badge" style="margin-bottom:8px">'+matChoice.mdfPlenkaPrice.toLocaleString('ru-RU')+' ₸ / м²</div>' +
        '<div class="fl">Краска (марка)</div><select class="cat-select" onchange="setMdfKraskaName(this.value)">'+kraskaOpts+'</select>' +
        '<div class="cat-price-badge">'+matChoice.mdfKraskaPrice.toLocaleString('ru-RU')+' ₸ / м²</div>' +
        '<div style="font-size:10px;color:#888;margin-top:6px">Обе марки доступны одновременно — у каждой секции/двери свой выбор Плёнка/Краска/ЛДСП.</div>';
    }
  }
...
function setMdfPlenkaName(name){
  const d=(catalog.facadePlenka||[]).find(x=>x.name===name);
  if(d){ matChoice.mdfPlenkaName=d.name; matChoice.mdfPlenkaPrice=d.price; }
  saveCatalog(); updateStats(); projMarkUnsaved();
}
function setMdfKraskaName(name){
  const d=(catalog.facadeKraska||[]).find(x=>x.name===name);
  if(d){ matChoice.mdfKraskaName=d.name; matChoice.mdfKraskaPrice=d.price; }
  saveCatalog(); updateStats(); projMarkUnsaved();
}
  ...
  sections.forEach((s,idx)=>{
      ...
      acc('facade','🎨 Фасад',facBadge,
        `<div class="g2" style="margin-bottom:6px">` +
          ...
          `<select onchange="updFacade(${s.id},'material',this.value)">` +
            `<option value="ldsp" ${normFacadeMat(s.facade.material)==='ldsp'?'selected':''}>ЛДСП</option>` +
        ...
        `<div class="g2" style="margin-bottom:6px">` +
          ...
          `<div><div class="fl">Материал</div>` +
            `<select onchange="updAntresol(${s.id},'facMat',this.value)">` +
              `<option value="ldsp" ${normFacadeMat(antr.facade.material)==='ldsp'?'selected':''}>ЛДСП</option>` +
              '<option value="mdfPlenka" '+(normFacadeMat(antr.facade.material)==='mdfPlenka'?'selected':'')+'>МДФ Плёнка</option>' +
              '<option value="mdfKraska" '+(normFacadeMat(antr.facade.material)==='mdfKraska'?'selected':'')+'>МДФ Краска</option>' +
            `</select></div>` +
        `</div>` +
    ...
    if(s.facade.type!=='none'){
      const doorRows=Array.from({length:doorCount},(_,di)=>{
        const curMat=normFacadeMat(s.facadeDoors[di]?s.facadeDoors[di].material:s.facade.material);
        return `<div class="facade-door-row">
          ...
          <select onchange="updDoorMat(${s.id},${di},this.value)">
            ...
            <option value="mdfPlenka" ${curMat==='mdfPlenka'?'selected':''}>МДФ Плёнка</option>
            <option value="mdfKraska" ${curMat==='mdfKraska'?'selected':''}>МДФ Краска</option>
          </select>
        </div>`;
      }).join('');
      ...
      try{
        ...
        function addHandle3d(p,b,cx,cy,cz){
          ...
          if(p.kind==='dfacade'){
            ...
            // Сторона открывания: явная настройка секции, иначе авто-зеркало —
            // створка в левой половине секции = петли слева, в правой = петли справа.
            let op=(s.facade&&s.facade.opening)||'';
            if(op!=='left'&&op!=='right') op=(b.cx<-5)?'left':'right';
            // петли слева → ручка у ПРАВОГО края; петли справа → ручка у ЛЕВОГО края
    ...
    sections.forEach(s=>{
      if(s.antresol&&s.antresol.enabled){
        ...
        if(s.antresol.facade.type!=='none'){
          const fm=isMdfMaterial(s.antresol.facade.material)?MFM:MFL;
...
/* ============================================================
   СБОРОЧНЫЙ ЧЕРТЁЖ
   Лист на каждый модуль: вид спереди с номерами позиций,
   деталировка, фурнитура, штамп. Отдельный лист фасадов.
   Данные — из ядра (те же box и cut, что уходят в раскрой).
============================================================ */

// Реквизиты проекта для штампа
function drawMeta(){
  function val(id, def){
    const el=document.getElementById(id);
    return (el&&el.value)?el.value:def;
  }
  const rawDate=val('proj-date-inp','');
  let dateStr;
  if(rawDate){ dateStr=new Date(rawDate).toLocaleDateString('ru-RU'); }
  else { dateStr=new Date().toLocaleDateString('ru-RU'); }
  return {
    name:   val('proj-name-inp','Шкаф'),
    client: val('proj-client-inp',''),
    phone:  val('proj-phone-inp',''),
    date:   dateStr
  };
}

// Геометрия модуля через ядро — тот же cfg, что в calcPartsCore/render3D
function coreCfgFor(s){
  return {
    width: s.width, height: s.height, depth: s.depth, legs: 0,
    panel: T, back: 3, edge: 1, gapFront: T, gapBack: 3,
    shelfDepthOffset: s.shelfDepthOffset || 0,
    sideExtraLeft: s.sideExtraLeft || 0,
    sideExtraRight: s.sideExtraRight || 0,
...
// Читаемое имя детали по kind ядра
const PART_RU = {
  side:'Стойка', top:'Крыша', bottom:'Дно', back:'Задняя стенка',
  shelf:'Полка', divider:'Перегородка', vert:'Перегородка',
  facade:'Фасад', dfacade:'Фасад ящика', dside:'Боковина ящика',
  dfront:'Перед ящика', dback:'Зад ящика', dbottom:'Дно ящика',
  dpost:'Стойка ящика', drail:'Планка ящика', rod:'Штанга'
};

function partMatRu(p){
  if(p.material==='metal') return 'Фурнитура';
  if(p.kind==='back'||p.kind==='dbottom') return 'ХДФ 3';
  return 'ЛДСП 16';
}

// Сводка позиций: одинаковые детали — одна строка с количеством
function posTableFor(parts){
  const agg=[];
  parts.forEach(function(p){
    const nm=PART_RU[p.kind]||p.kind;
    const w=(p.kind==='side')?p.cutW:p.cutL;
    const h=(p.kind==='side')?p.cutL:p.cutW;
    const mat=partMatRu(p);
    const key=nm+'|'+w+'|'+h+'|'+mat;
    let row=agg.find(function(r){ return r.key===key; });
    if(!row){
      row={key:key, name:nm, w:w, h:h, mat:mat, qty:0, parts:[]};
      agg.push(row);
    }
    row.qty++;
    row.parts.push(p);
  ...
  agg.forEach(function(r,i){
    r.pos=i+1;
    r.parts.forEach(function(p){ p._pos=i+1; });
  });
  return agg;
}

// Вид спереди модуля с выносками позиций
function moduleSvg(s, parts, W, H){
  const PAD=54;
  const VW=620, VH=560; // +100 — колонки выносок слева/справа
  const sc=Math.min((VW-PAD*2-70)/W, (VH-PAD*2)/H);
  const ox=(VW-W*sc)/2, oy=PAD;
  const X=function(x){ return ox+x*sc; };
  const Y=function(y){ return oy+(H-y)*sc; };
  let g='';
  // детали: box ядра — cx/cy центр, dx/dy габарит
  parts.forEach(function(p){
    if(!p.box) return;
    if(p.material==='metal') return;
  ...
  // выноски — по одной на позицию, вынесены за габарит с линией-выноской,
  // чтобы не налезали друг на друга и не закрывали геометрию
  const marks=[];
  const seen={};
  parts.forEach(function(p){
    if(!p.box||!p._pos||seen[p._pos]) return;
    if(p.material==='metal') return;
    seen[p._pos]=1;
    marks.push({pos:p._pos, x:X(p.box.cx), y:Y(p.box.cy)});
  });
  // чётные — слева, нечётные — справа; шаг по вертикали фиксированный
  marks.sort(function(m1,m2){ return m1.y-m2.y; });
  const left=[], right=[];
  marks.forEach(function(m,i){ if(i%2===0) left.push(m); else right.push(m); });
  const STEP=22, R=8.5;
  function place(list, colX){
    list.sort(function(m1,m2){ return m1.y-m2.y; });
    let prev=-1e9;
    list.forEach(function(m){
      let ly=Math.max(m.y, prev+STEP);
      prev=ly;
      m.lx=colX; m.ly=ly;
    });
  }
  place(left, ox-26);
  place(right, ox+W*sc+26);
  marks.forEach(function(m){
    g+='<line x1="'+m.x.toFixed(1)+'" y1="'+m.y.toFixed(1)+'" x2="'+m.lx.toFixed(1)+'" y2="'+m.ly.toFixed(1)
      +'" stroke="#8a8f96" stroke-width="0.4"/>'
...
function moduleHardware(s){
  ...
  if(s.facade.type!=='none'){
    ...
    hinges+=dc*((s.height-8)>1500?3:2);
    handles+=dc;
  }
  if(s.antresol&&s.antresol.enabled&&s.antresol.facade.type!=='none'){
    const ac=s.antresol.facade.type==='doors3'?3:s.antresol.facade.type==='doors2'?2:1;
    hinges+=ac*(s.antresol.height>1500?3:2);
    handles+=ac;
  }
  let drawers=0;
  if(s.drawerBlocks&&s.drawerBlocks.length){
    const cols=getColumns(s).length;
    s.drawerBlocks.forEach(function(db){
      const n=db.count*((db.col!=null&&db.col>=0)?1:cols);
      drawers+=n; handles+=n;
    });
  }
  add('Петля '+activehingeBrand, hinges, 'шт');
  add('Ручка', handles, 'шт');
  if(drawers>0){
...
function drawStamp(meta, title, sub, gab){
  return '<div class="stamp">'
    +'<div class="st-l"><div class="st-brand">MEBELOFF</div>'
    +'<div class="st-sm">Изделие по индивидуальному проекту</div>'
    +(meta.client?('<div class="st-sm">Клиент: '+meta.client+'</div>'):'')
    +(meta.phone?('<div class="st-sm">Тел: '+meta.phone+'</div>'):'')
    +'</div>'
    +'<div class="st-c"><div class="st-title">'+title+'</div>'
    +'<div class="st-sm">'+sub+'</div></div>'
    +'<div class="st-r"><div class="st-sm">Габариты (Ш×В×Г)</div>'
    +'<div class="st-gab">'+gab+'</div>'
    +'<div class="st-sm">Дата: '+meta.date+'</div>'
    +'<div class="st-sm">Проект: '+meta.name+'</div></div>'
    +'</div>';
}
...
function showAssemblyDrawing(){
  ...
  sections.forEach(function(s,i){
    ...
    let fBody;
    if(facades.length){
      const fPos=posTableFor(facades);
      const fTbl=drawTable(['Поз.','Название','Размер','Кол.','Материал'],
        fPos.map(function(r){ return [r.pos, r.name, r.w+'×'+r.h, r.qty, r.mat]; }));
      ...
      fBody='<div class="empty">Фасадов нет</div>';
    }
    pages+='<div class="page">'
      +drawStamp(meta,'Фасады','Модуль №'+(i+1),gab)
      +'<div class="body">'+fBody+'</div>'
      +'<div class="foot">Лист '+pageNo+' / '+total+'</div></div>';
  });

  const css='@page{size:A4 landscape;margin:8mm}'
    +'body{font-family:Arial,Helvetica,sans-serif;margin:0;color:#1d2023}'
    +'.page{page-break-after:always;padding:4mm;border:1px solid #1d2023;margin-bottom:6mm}'
    +'.stamp{display:flex;border-bottom:2px solid #1d2023;padding-bottom:3mm;margin-bottom:3mm}'
    +'.st-l{flex:1}.st-c{flex:1;text-align:center}.st-r{flex:1;text-align:right}'
    +'.st-brand{font-size:14pt;font-weight:800;letter-spacing:2px;color:#b8963e}'
    +'.st-title{font-size:15pt;font-weight:700}'
    +'.st-gab{font-size:12pt;font-weight:700}'
    +'.st-sm{font-size:8pt;color:#565c63}'
    +'.body{display:flex;gap:5mm}'
    +'.col-svg{flex:1.1;border:1px solid #d8d5cd;padding:2mm}'
    +'.col-tbl{flex:1}'
    +'.tbl-h{font-size:10pt;font-weight:700;margin:2mm 0 1mm}'
    +'.dt{width:100%;border-collapse:collapse;font-size:8pt}'
    +'.dt th{background:#f2f0eb;border:1px solid #c9c5bb;padding:1mm;text-align:left}'
    +'.dt td{border:1px solid #d8d5cd;padding:1mm}'
    +'.note{font-size:8.5pt;color:#565c63;margin-top:3mm;line-height:1.5}'
    +'.empty{padding:20mm;text-align:center;color:#8a8f96}'
    +'.foot{margin-top:2mm;font-size:8pt;color:#8a8f96;text-align:right}'
    +'@media print{.noprint{display:none}.page{border:none}}';
  const html='<!DOCTYPE html><html><head><meta charset="utf-8"><title>Сборочный чертёж</title><style>'+css+'</style></head><body>'
    +'<div class="noprint" style="padding:8px;background:#f2f0eb;margin-bottom:6px">'
    +'<button onclick="window.print()">Печать / PDF</button> Листов: '+total+'</div>'
    +pages+'</body></html>';
...
window.showAssemblyDrawing=showAssemblyDrawing;
...
function mdfSheetsBlockHtml(list, mTot, label, dotColor){
  if(!list.length) return '';
  return '<div class="cut-mat-block">' +
    '<div class="cut-mat-title"><span class="color-dot" style="background:'+dotColor+';border:1px solid #ccc"></span>'+label+' — расчёт по площади</div>' +
    '<div class="mdf-block">' +
      list.map(function(p){
...
function showCut(){
  ...
  sheetsHtml+=mdfSheetsBlockHtml(facMdfPlenka, mdfPlenkaM2, 'МДФ Плёнка фасады', '#fff0d4');
  sheetsHtml+=mdfSheetsBlockHtml(facMdfKraska, mdfKraskaM2, 'МДФ Краска фасады', '#e8dcc8');
  ...
  const html=`
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">
      <button class="btn" onclick="showAssemblyDrawing()">📐 Сборочный чертёж</button>
...
function renderMobile3D(){
  ...
  sections.forEach(s=>{
    ...
    if(s.facade.type!=='none'){
      const fm=isMdfMaterial(s.facade.material)?mMFM:mMFL;
      const count=s.facade.type==='doors3'?3:s.facade.type==='doors2'?2:1;
      const gap=4,thick=T,dw=(W-gap*(count+1))/count;
    ...
    if(s.antresol&&s.antresol.enabled){
      ...
      if(s.antresol.facade.type!=='none'){
        const fm=isMdfMaterial(s.antresol.facade.material)?mMFM:mMFL;
        const cnt=s.antresol.facade.type==='doors3'?3:s.antresol.facade.type==='doors2'?2:1;
...
window.setLdsp=setLdsp; window.setMdfPlenkaName=setMdfPlenkaName; window.setMdfKraskaName=setMdfKraskaName;
