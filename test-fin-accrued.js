...
const ORDERS = [
  { num:'46', status:'Договор', client:'уцка', furn:'Кухня', sogl:302040, pred:302040,
    ...
    earnMaster:45306, earnDesigner:0, masterId:'e2', helperId:'', helperPay:0, updated:D }
];
const RECUR = [
...
const FIN = [
  ...
  // v4.14: процент начислен тем же тегом, что ставит accrueMonth_
  { id:'a5', date:D, type:'Расход', cat:'Оплата мастеру',   sum:45306,  num:'', comment:'[Процент ' + MONTH + ' #e2] Серик' }
];
  ...
  setTimeout(function(){
    ...
    setTimeout(function(){
      ...
      setTimeout(function(){
        ...
        ok(exp === -745306, 'расход за месяц = 745 306\u20B8 — постоянные, оклады И процент проведены');
        // 302 040 − 745 306 = −443 266. Ровно столько же, сколько в
        // test-fin.js БЕЗ начисления: честный итог НЕ зависит от того,
        // нажата кнопка или нет. Это и есть смысл всей правки.
        ok(net === -443266, 'чистый доход = \u2212443 266\u20B8 — тот же, что и без начисления');
        ok(!/\u041d\u0435 \u043d\u0430\u0447\u0438\u0441\u043b\u0435\u043d\u043e \u043e\u0431\u044f\u0437\u0430\u0442\u0435\u043b\u044c\u0441\u0442\u0432/.test(txt),
          'предупреждения о недостаче НЕТ — процент теперь закрывается кнопкой, вечной плашки не осталось');
        ok(!/\u0431\u0435\u0437 \u043d\u0430\u0437\u043d\u0430\u0447\u0435\u043d\u043d\u043e\u0433\u043e \u043c\u0430\u0441\u0442\u0435\u0440\u0430/.test(txt),
          'мастер назначен — предупреждения о нераспределённых деньгах нет');
